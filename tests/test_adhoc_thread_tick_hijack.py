...
never claim/re-dispatch an ad-hoc (conversation-origin) thread — even when a
tick-minted dispatch title happens to lexically match it.
...
juggle_topic_lifecycle.reconcile_adhoc_integrate) sat live in project INBOX.
A later graph_tick, dispatching an unrelated graph topic whose minted title
(``f"[{tid}] {topic['title']}"``) shared 2+ significant content words with the
ad-hoc thread's own title, was reused by db.create_thread's lexical dedup
guard (dbops.thread_dedup._find_duplicate_open_thread) — which only excludes
threads that already OWN a *different* active topic/task, not threads that
are ad-hoc (own NOTHING). The tick then bound + re-dispatched INTO the ad-hoc
thread: retitling it, sending the tick's task prompt into an agent already
mid-flight on unrelated ad-hoc work (run misattribution).

RED pre-fix: the tick's dispatch lands on the ad-hoc thread instead of a
...
def test_tick_does_not_hijack_a_lexically_similar_adhoc_thread(db):
    # The orchestrator's own ad-hoc, live conversation — no db_graph task and
    # no db_topics topic binding (the ad-hoc discriminator's definition).
    adhoc_id = db.create_thread(
        "stall nudge false positive investigation", session_id="s",
        project_id="INBOX",
    )
    db.set_conversation_background(adhoc_id)  # "live" ad-hoc work in flight
