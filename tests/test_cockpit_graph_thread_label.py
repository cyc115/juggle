"""Graph panel shows readable A-Z topic label on in-progress tasks."""
import sys
...
def test_running_task_shows_user_label_not_thread_uuid():
    task = GraphTask(
        id="footer-narrow", title="t", state="running",
        thread_id="ff1e5095-819c-4629", user_label="WR",
    )
    txt = _cell_text(1, task, idx_w=2, dep_num=None, cell_w=40, selected=False).plain
