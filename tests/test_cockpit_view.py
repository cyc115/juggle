...
def _make_actions():
    return [
        ...
        Action(
            id="a2", topic_id="B", text="BLOCKER: missing token", tier=0, age_secs=7200
        ),
        Action(
            id="a3", topic_id="I", text="survey results ready", tier=1, age_secs=1800
        ),
    ]
...
def _make_agents():
    return [
        Agent(
            id_short="abcd1234", role="coder", status="busy", topic_id="K", age_secs=720
        ),
        Agent(
            id_short="ef567890",
            role="planner",
