...
def test_dry_run_writes_digest(tmp_path):
    mock_db = MagicMock()
    with (
        patch.object(reflect, "get_db", return_value=mock_db),
        patch.object(common, "gh_pr_list_head", return_value=[]),
        patch.object(
            reflect,
            "rf1_watchdog",
            lambda db, ct, s: s.update({"RF-1": "## Watchdog Health\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf2_action_items",
            lambda db, ct, s: s.update({"RF-2": "## Action Item Fatigue\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf3_completion_quality",
            lambda db, ct, s: s.update({"RF-3": "## Agent Output Quality\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf4_context_bloat",
            lambda db, s: s.update({"RF-4": "## Context Bloat Candidates\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf5_hindsight_lint",
            lambda ct, s: s.update({"RF-5": "## Memory Health\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf6_auto_memory",
            lambda ct, s: s.update({"RF-6": "## Auto-Memory Contradictions\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf7_skill_drift",
            lambda db, ct, s: s.update({"RF-7": "## Skill Drift\n\nOK\n"}),
        ),
        patch.object(
            reflect,
            "rf8_dogfood_pulse",
            lambda s: s.update({"RF-8": "## Dogfood Pulse\n\nOK\n"}),
        ),
...
def test_issue_cap_5(tmp_path):
    ...
    with (
        patch.object(common, "gh_issue_exists", return_value=False),
        patch.object(common, "gh_create_issue", side_effect=fake_create),
    ):
        reflect._file_reflect_issues(
            sections, "2026-05-18", tmp_path / "r.md", dry_run=False
        )
...
def test_issue_dedup_skips_existing(tmp_path):
    ...
    with (
        ...
        patch.object(
            common, "gh_create_issue", side_effect=lambda *a, **kw: filed.append(a)
        ),
    ):
        reflect._file_reflect_issues(
            sections, "2026-05-18", tmp_path / "r.md", dry_run=False
...
def test_rf4_no_messages_table():
    ...
    sections = {}
    with patch.object(
        reflect, "db_query", side_effect=Exception("no such table: messages")
    ):
...
def test_find_autofix_pr_ref_with_pr():
    with patch.object(
        reflect, "gh_pr_list_head", return_value=[{"number": 99, "state": "OPEN"}]
    ):
        ref = reflect._find_autofix_pr_ref()
    assert "99" in ref
...
def test_reflect_cost_cap_dry_run_does_not_raise(tmp_path):
    ...
    with (
        ...
        patch.object(
            reflect,
            "rf4_context_bloat",
            lambda db, s: s.update({"RF-4": "## RF-4\n\nOK\n"}),
        ),
