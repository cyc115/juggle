...
def test_canary_spawn_calls_cli_commands():
    ...
    def fake_run(cmd, **kwargs):
        ...
        if "get-agent" in cmd:
            ...
            mock_result.stdout = json.dumps([{"id": "agent-abc12345", "status": "idle"}])
        elif "create-thread" in cmd:
            mock_result.stdout = "Created Topic JA: dogfood-dryrun-2026-05-19. Now in Topic JA.\n"
        else:
            mock_result.stdout = ""
        mock_result.stderr = ""
        return mock_result
    ...
    with patch("juggle_schedule_dogfood.subprocess.run", side_effect=fake_run), \
