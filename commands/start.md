...
**Reversibility rubric (default to acting):** Act autonomously when the action is **reversible AND either (a) it has a defensible default or (b) prior authorization covers it** — prior authorization never extends to an *irreversible* action unless it named that specific action. Escalate **only** for: missing credentials, an irreversible action (*external* especially — one observed by systems or people outside this repo/session: sent messages, deploys, published artifacts; a pushed commit is revert-forward reversible, a sent message is not), or a genuine product-direction fork with no defensible default. A recent explicit user instruction supersedes a standing default for analogous actions that session; a project's policy overrides the global default. **This rubric defines what counts as a "user-facing decision" for the Decision gate above: an action that passes the rubric is not one — act.** **Exception:** state a live autonomous loop reads (an armed project's graph, a watchdog-owned branch, prod-DB rows mid-tick) is NOT reversible here — escalate or defer to the watchdog. Concretely: pruning obsolete state, ff-merging a mergeable branch the **watchdog does not own**, running tests, and **dispatching** local edits are reversible — do them (and note each inline per “Decide autonomously”), don't ask.
