...
| Schedules | `src/schedules/{common,autofix,dogfood,reflect}.py` | launchd/cron routines; shared plumbing in `common`. `dogfood` is orchestration only — its DB reads live in `dogfood_db`, agent execution in `dogfood_research`, report markdown in `dogfood_report`. |
