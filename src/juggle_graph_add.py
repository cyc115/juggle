"""
juggle_graph_add — validated, atomic, guarded single-node add into a live graph.

Backs `juggle graph add-node` (juggle_cmd_graph.cmd_graph_add_node): inject ONE
new task node into an EXISTING project graph mid-execution without disarming or
restarting. Split out of juggle_graph_upsert (2026-06-10) to keep both modules
under the 300-line architecture gate.

Owns: the single-node add path (validate_add_node + add_node + the readiness
demotion helper) and its guard rules (which existing-node states a new edge may
touch).
Must not own: node state semantics (dbops.db_graph — node_transition stays the
...
def add_node(
    ...
    *,
    ...
    # the dispatcher claims any node in state='ready' without re-checking deps,
    # so a crash between commit and a post-commit demote could leave a 'ready'
    # node with an unverified dep and dispatch it prematurely.
    conn = db._connect()
    try:
        if node_id in live:
        ...
        for rb in required_by:
            ...
            if live[rb]["state"] == "ready":
                db_graph.node_transition(db, rb, "unready", conn=conn)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
    ...
    # Promote the new node (and resolve any blocked tail) through the existing
    # seams. Promotion lagging a crash is safe — the node merely waits a tick.
