"""juggle_graph_status — read-only display aggregates over graph_nodes.

Owns: per-project node-state counts, the cockpit progress string
("3/14 done, 1 failed, 2 ready" — DA m2), and the char-budgeted
UserPromptSubmit graph-status injection (HARD 500 chars, DA m4).
Must not own: node state semantics (dbops.db_graph), dispatching
...
def counts_from_states(states: list[str]) -> dict:
    """Aggregate raw graph_nodes.state values into display counts. Pure."""
    return {
        "total": len(states),
...
def graph_counts(db, project_id: str) -> dict | None:
    """Counts for ``project_id``, or None (no nodes / pre-migration DB)."""
    try:
        with db._connect() as conn:
            rows = conn.execute(
                "SELECT state FROM graph_nodes WHERE project_id=?", (project_id,)
            ).fetchall()
    except Exception:
        return None  # pre-migration DB without graph_nodes
...
def _titled(db, project_id: str, states: tuple[str, ...]) -> list[tuple[str, str]]:
    with db._connect() as conn:
        ph = ",".join("?" * len(states))
        rows = conn.execute(
            f"SELECT id, title FROM graph_nodes WHERE project_id=? "
...
def build_graph_injection(db, project_id: str, budget: int = INJECTION_BUDGET) -> str:
    ...
    Counts + ready/running node titles; truncation is deterministic (fixed
