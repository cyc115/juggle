...
dataclasses. Edges are (task_id, depends_on_id) pairs (arrow points at the
...
def assign_ranks(tasks: list[GraphTask], edges: list[tuple[str, str]]) -> dict[str, int]:
    """Map task id -> rank, rank = longest dependency depth (longest path).

    Cycle-guarded: iterates to a fixpoint, capped at len(tasks) passes so a
    malformed cyclic input terminates with finite ranks instead of hanging.
    """
    ids = {n.id for n in tasks}
    deps: dict[str, list[str]] = {n.id: [] for n in tasks}
    for task_id, dep_id in edges:
        if task_id in ids and dep_id in ids:
...
def build_ranks(tasks: list[GraphTask], edges: list[tuple[str, str]]) -> list[Rank]:
    """Group tasks into ordered Rank columns (stable by task id within a rank)."""
    rank_of = assign_ranks(tasks, edges)
    by_rank: dict[int, list[GraphTask]] = {}
    for n in tasks:
        by_rank.setdefault(rank_of[n.id], []).append(n)
    out: list[Rank] = []
    for idx in sorted(by_rank):
        cells = sorted(by_rank[idx], key=lambda c: c.id)
        out.append(Rank(index=idx, tasks=cells))
    return out
...
def _rank_is_active(rank: Rank) -> bool:
    """A rank is 'active' (keep expanded) if any task is ready/running/failed."""
    return any(
        any(n.state.startswith(p) for p in _ACTIVE_PREFIXES) for n in rank.tasks
    )
...
def _fold(rank: Rank) -> Rank:
    ...
    label = f"…{len(rank.tasks)} {dominant}"
    return Rank(index=rank.index, tasks=list(rank.tasks), collapsed=True, label=label)
...
def collapse_ranks(ranks: list[Rank], max_visible_ranks: int) -> list[Rank]:
    ...
    if max_visible_ranks <= 0 or len(ranks) <= max_visible_ranks:
        return [Rank(index=r.index, tasks=list(r.tasks)) for r in ranks]

    # Fold every non-active rank whose tasks are all foldable.
    result: list[Rank] = []
    for r in ranks:
        foldable = all(n.state in _FOLDABLE for n in r.tasks) and not _rank_is_active(r)
        result.append(_fold(r) if foldable else Rank(index=r.index, tasks=list(r.tasks)))
    ...
    if collapsed:
        ...
        total = sum(len(r.tasks) for r in collapsed)
        ...
        summary = Rank(
            ...
            tasks=[n for r in collapsed for n in r.tasks],
            collapsed=True,
