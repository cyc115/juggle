...
def assign_project_background(
    ...
    script = (
        ...
        "pid != INBOX_PROJECT_ID and (db.update_thread({thread_uuid!r}, project_id=pid, assigned_by='auto', assigned_confidence=conf) or True) "
...
def _assign_thread_to_project(
    ...
    # P8 (Task 4.2): no graph_topics mirror to upsert/delete — the conversation IS
    # a kind='conversation' node carrying its own project_id (db.update_thread mirrors
    # it onto the node), so project assignment needs no separate projection.
