...
def clear_stale_unmerged_escalations(db, node_id: str) -> list[int]:
    ...
    Called from the reconcile success path (``reconcile_out_of_band_merges``)
    after a topic is stamped verified+merged_sha. Items are matched EXACTLY via
    the ``action_item_id`` recorded on each ``_ORPHAN_EVENT`` marker at filing
    time (orphan_guard.flag_unmerged_completed_topics) — never by grepping the
    message. Emits one INFO notification when anything was cleared.

    Incident (2026-07-03/04 integrate-wedge #2 follow-up): the watchdog filed the
    UNMERGED escalation but nothing closed it when the merge subsequently landed,
    so item 5308 stayed open for the operator to hand-ack.
    """
    with db._connect() as conn:
