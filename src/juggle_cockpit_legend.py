...
STATUS_LEGEND: list[dict] = [
    ...
    {"section": "Graph (task DAG)", "entries": [
        # Richer-state palette (2026-07-03): colour = state (redundant with the
        # glyph shape), rail connectors inherit their upstream node's colour.
        {"glyph": TASK_STATE_GLYPHS["open"],           "meaning": "open — blocked on deps (grey)"},
        {"glyph": TASK_STATE_GLYPHS["ready"],          "meaning": "ready — next up (cyan)"},
        {"glyph": TASK_STATE_GLYPHS["dispatching"],    "meaning": "dispatching (sky-blue)"},
        {"glyph": TASK_STATE_GLYPHS["running"],        "meaning": "running (gold)"},
        {"glyph": TASK_STATE_GLYPHS["integrating"],    "meaning": "integrating (blue); unlanded = purple"},
        {"glyph": TASK_STATE_GLYPHS["verified"],       "meaning": "verified — done (green)"},
        {"glyph": TASK_STATE_GLYPHS["failed-exec"],    "meaning": "failed — exec (red3) / integration (magenta) / verify (red1)"},
        {"glyph": TASK_STATE_GLYPHS["blocked-failed"], "meaning": "blocked by a failed dependency (amber)"},
